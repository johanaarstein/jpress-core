...
let moreInfo_str: string

let iAccept_str: string
let cookiesSettings_str: string
let functionality_str: string
let performance_str: string
let showLess_str: string
let soMe_str: string
let privacy_str: string
let jpCW_str1: string
let jpCW_str2: string
let jpCW_str3: string
let jpCW_str4: string
let jpCW_str5: string
...
jpressCookieWarning.prototype.init = function () {
  const isPrivacyUrl = window.location.pathname === this.privacyUrl || window.location.href === this.privacyUrl
...
jpressCookieWarning.prototype.execScripts = function () {
  for (const option in this.preferences) {
    ...
    if (this.hasRun[option] === false) {
      ...
      const funcName = `sempro${  option.charAt(0).toUpperCase()  }${option.slice(1)}`
...
jpressCookieWarning.prototype.save = function (options) {
  if (supportsHtml5Storage() && isLocalStorageNameSupported()) {
    if (options) {
      ...
      const inputs = document.querySelectorAll('#tplPreferences input[type="checkbox"]')

      for(const input of inputs) {
        if(input.name !== '') {
          this.preferences[input.name] = Boolean(input.checked)
        }
      }
...
jpressCookieWarning.prototype.close = function () {
  const el = document.getElementById('jp-cw-mask')
