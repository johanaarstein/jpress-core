  ...
  <div class="wrapper">
    ...
    <?php
      if (isset($_GET['reset'])) {
        ...
        var request=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");var requestString,resetForm=document.getElementById("reset-form"),recaptchaResponse=document.getElementById("recaptchaResponse"),resetBtn=document.getElementById("reset-button");if(resetForm){var onloadCallback=function(){grecaptcha.execute("<?php echo $reCAPTCHA_siteKey; ?>",{action:"reset"}).then(function(a){recaptchaResponse.value=a})};resetBtn.addEventListener("click",function(a){a.preventDefault(),requestString="reset-password&recaptcha_response="+recaptchaResponse.value,request.onreadystatechange=function(){4===this.readyState&&(200<=this.status&&300>this.status?resetForm.submit():400<=this.status&&600>this.status&&(this.responseText?(resetForm.insertAdjacentHTML("beforeBegin","<p class=\"aligncenter\">"+this.responseText+"</p>"),console.log(this.responseText)):resetForm.insertAdjacentHTML("beforeBegin","<p class=\"aligncenter\"><?php echo $thereWasAnError_str; ?></p>")))},request.open("POST","/jp-includes/plugins/recaptcha/recaptcha.php",!0),request.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=UTF-8"),request.send(requestString)},!1)}
        </script>
      <?php } else { ?>
        <script <?php echo nonce() ? 'nonce="' . NONCE . '"' : ''; ?>>
